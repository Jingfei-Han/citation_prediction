...
class extractCitation(object):
    ...
    def _parseGoogle(self, response):
        ...
        for paper in soup.find_all("div", attrs={"class": "gs_r"}):
            ...
            # 有些引文没有链接
            try:
                targetPaper_title = title.a.text
                targetPaper_rawURL = title.a['href']
            ...
            try:
                related_raw = paper.find("a", text=re.compile("Related")) # 获取相关文章链接
                targetPaper_relatedURL = related_raw.get('href')
            except:
                targetPaper_relatedURL = ""
            ...
            self.addSQL(targetPaper_title, targetPaper_scholarInfo, targetPaper_citationURL, targetPaper_relatedURL,
                                 targetPaper_pdfURL, targetPaper_rawURL, targetPaper_rawInfo,
                                 targetpaper_publicationYear, targetPaper_nbCitation, targetPaper_isseen,
                                 paper_paper_id)

    def addSQL(self, targetPaper_title, targetPaper_scholarInfo, targetPaper_citationURL, targetPaper_relatedURL, targetPaper_pdfURL, targetPaper_rawURL, targetPaper_rawInfo, targetpaper_publicationYear, targetPaper_nbCitation, targetPaper_isseen, paper_paper_id):
        # 无论解析阶段是否出现异常都写入数据库
        if self.sql_cnt != 0:
            self.sql += ","
        self.sql += "('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%d', '%d', '%d', '%d')" % (
                         targetPaper_title.replace('\'', '\\\'').strip(), targetPaper_scholarInfo.replace('\'', '\\\'').strip(),
                         targetPaper_citationURL.replace('\'', '\\\'').strip(),
                         targetPaper_relatedURL.replace('\'', '\\\'').strip(),
                         targetPaper_pdfURL.replace('\'', '\\\'').strip(),
                         targetPaper_rawURL.replace('\'', '\\\'').strip(),
    ...
    def executeDataBase(self):
        ...
        if self.sql_cnt == 0:
            return
        sql_update = "INSERT INTO targetpaper(targetPaper_title, targetPaper_scholarInfo, " \
                     "targetPaper_citationURL, targetPaper_relatedURL, targetPaper_pdfURL, targetPaper_rawURL, targetPaper_rawInfo, " \
